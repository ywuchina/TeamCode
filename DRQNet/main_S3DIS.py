...
class S3DIS:
    ...
    def load_sub_sampled_clouds(self, sub_grid_size, labeled_point, retrain):
        ...
        for i, file_path in enumerate(self.all_files):
            ...
            if cloud_split == 'training':
                ...
                # =================================================================== #
                if retrain:
                    pseudo_label_path = 'results/test'
            ...
            for i in range(num_per_epoch):  # num_per_epoch
                ...
                if split == 'training':
                    ...
                    if len(unique_label_value) <= 1:
                        ...
                        # color_with_anno = queried_pc_colors[idx_with_anno]
                ...
                if True:
                    yield (queried_pc_xyz.astype(np.float32),
                           ...
                           # color_with_anno.astype(np.float32),
    ...
    def get_tf_mapping2():
        ...
        def tf_map(batch_xyz, batch_rgb, batch_labels, batch_pc_idx, batch_cloud_idx, batch_xyz_anno, batch_label_anno):
            batch_features = tf.concat([batch_xyz, batch_rgb], axis=-1)
            # batch_features = batch_rgb
            input_xyz = []
            input_rgb = []
            input_neighbors = []
            input_pools = []
            ...
            for i in range(cfg.num_layers):
                ...
                sub_xyz = batch_xyz[:, :tf.shape(batch_xyz)[1] // cfg.sub_sampling_ratio[i], :]
                sub_rgb = batch_rgb[:, :tf.shape(batch_rgb)[1] // cfg.sub_sampling_ratio[i], :]
                pool_i = neighbour_idx[:, :tf.shape(batch_xyz)[1] // cfg.sub_sampling_ratio[i], :]
                up_i = tf.py_func(DP.knn_search, [sub_xyz, batch_xyz, 1], tf.int32)
                input_xyz.append(batch_xyz)
            ...
            input_list = input_xyz + input_rgb + input_neighbors + input_pools + input_up_samples
            input_list += [batch_features, batch_labels, batch_pc_idx, batch_cloud_idx, batch_xyz_anno, batch_label_anno]
...
if __name__ == '__main__':
    ...
    parser.add_argument('--exp', type=str, default='exp_s3dis', help='experiment name')
    parser.add_argument('--gpu', type=str, default='0', help='GPUs to use [default: 0]')
    ...
    print('Labeled_point:', args.labeled_point)
    print('gen_pseudo:', args.gen_pseudo)
    ...
    dataset = S3DIS(test_area, args.labeled_point, args.retrain)
    dataset.init_input_pipeline()

    mirrored_strategy = tf.distribute.MirroredStrategy(devices=["/gpu:0", "/gpu:1"])
    if Mode == 'train':
        model = Network(dataset, cfg, args)
        model.train(dataset)
    elif Mode == 'test':
        cfg.saving = False
        model = Network(dataset, cfg, args)
        chosen_snapshot = -1
        logs = np.sort([os.path.join('results/checkpoints', f) for f in os.listdir('results/checkpoints') if f.endswith(args.exp)])
        ...
        tester.test(model, dataset, args, args.gen_pseudo)
        shutil.rmtree('train_log') if exists('train_log') else None
    else:
