function [best] = PSTmethod(prm,d,proportion,G)
%根据输入的数据选择出最优的特征子集
%   prm是参数
    tic
    ...
    tasks=generateTask2(d,prm,proportion);
...
%                 真实评估
%                 tfits=eva4([o1{j};o2{j};o3{j}],prm2);
%                 fitss(j,(ppp(j)+1):(ppp(j)+length(nnfits)))=nnfits';
%                 ppp(j)=ppp(j)+length(nnfits);
%                 truefits(j,(ppp(j+tn)+1):(ppp(j+tn)+length(tfits)))=tfits';
%                 ppp(j+tn)=ppp(j+tn)+length(tfits);
