function getAllF(Maxnum)
%读取所有的特征
    AllF=[];
    pallF=cell(0);
    FF=cell(0);
    for i=1:Maxnum
        str1="Data\feature\p"+i+"_r20_";
%         str1="Data\feature\p"+i+"_";
...
%         cpallF{i}=f;
%     end
    save("Data\feature\AllFeature.mat",'pallF');
%     for i=1:Maxnum
%         p(i)=pcread("data\p1"+i+".ply");
%     end
%     T=load('data\T','T').T;
%     yuzhi=getYuzhi(p(1));
%     ac=zeros(1,6);
...
%             end
%             cs=cs+1;
%             [a,m]=test(i,j,gettranT(T{i},T{j}),yuzhi);
%             ac=ac+a;
%             mnum=mnum+m;
%         end 
%     end
%     ac=ac/cs;mnum=mnum/cs;
%     save("feature\AC.mat",'ac','mnum','yuzhi');
end
...
%     ropsF=processRopsData(importdata(str1+'rops'+str2));save("feature\ropsF_p"+i+".mat",'ropsF');
    ...
    fpfhF=processFPFHData(importdata(str1+'fpfh'+str2));save("Data\feature\p"+i+"_fpfhF.mat",'fpfhF');
    %pfhF=processPFHData(importdata(str1+'pfh'+str2));save("feature\p"+i+"_pfhF.mat",'pfhF');
    %rsdF=processRSDData(importdata(str1+'rsd'+str2));save("feature\p"+i+"_rsdF.mat",'rsdF');
    %uscF=processUSCData(importdata(str1+'usc'+str2));
    shotF=processShotData(importdata(str1+'shot'+str2));save("Data\feature\p"+i+"_shotF.mat",'shotF');
    siF=processSpinImageData(importdata(str1+'SpinImage'+str2));save("Data\feature\p"+i+"_siF.mat",'siF');
    ropsF=processRopsData(importdata(str1+'rops'+str2));save("Data\feature\p"+i+"_ropsF.mat",'ropsF');
