...
class SVDHead(nn.Module):
    ...
    def forward(self, *input):
        ...
        R = torch.stack(R, dim=0).to(torch.device('cuda:{}'.format(self.args.gpu)))
...
class PRNet(nn.Module):
    ...
    def _train_one_epoch(self, epoch, train_loader, opt):
        ...
        for data in tqdm(train_loader):
            src, tgt, rotation_ab, translation_ab, rotation_ba, translation_ba, euler_ab, euler_ba = [d.to(torch.device('cuda:{}'.format(self.args.gpu)))
                                                                                                      for d in data]
