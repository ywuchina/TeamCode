...
def main():
    ...
    ##### load data #####
    trainset = RegistrationkittiData(Kitti(split='train', type=args.type), sample_point_num=1024, is_testing=False)
    train_loader = DataLoader(trainset, batch_size=args.batch_size, shuffle=True, drop_last=True, num_workers=4)
    testset = RegistrationkittiData(Kitti(split='test', type=args.type), sample_point_num=1024, is_testing=True)
