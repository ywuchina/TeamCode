...
def load_data(partition):
    ...
    for h5_name in glob.glob(os.path.join(DATA_DIR, 'modelnet40_ply_hdf5_2048', 'ply_data_%s*.h5' % partition)):
        ...
        label = f['label'][:].astype('int64')  # (2048, 1) <class 'numpy.ndarray'>
        f.close()
...
class ModelNet40(Dataset):
    ...
    def __getitem__(self, item):
        ...
        R_ba = R_ab.T
        translation_ab = np.array([np.random.uniform(-1, 1), np.random.uniform(-1, 1),
...
class Registration3dmatchData(Dataset):
    def __init__(self, data,sample_point_num=2048,max_angle=45, max_t=1.0,
    ...
    def __getitem__(self, item):
        pointcloud = self.data[item][:self.sample_point_num]
