...
def test_one_epoch(args, net, test_loader):
    ...
    for src, target, rotation_ab, translation_ab, rotation_ba, translation_ba, euler_ab, euler_ba in tqdm(test_loader):
        ...
        # print(src.__class__,src.shape)  # <class 'torch.Tensor'> torch.Size([1, 3, 1024])
...
def train(args, net, train_loader, test_loader):
    ...
    scheduler = MultiStepLR(opt, milestones=[30, 50, 80], gamma=0.1)

    best_test_loss = np.inf
    wandb.init(project='DCP', name=args.exp_name)
    wandb.watch(net)
    for epoch in range(args.epochs):
        ...
        if best_test_loss >= test_loss:
            ...
            torch.save(net, '/home/ljm/dcp/checkpoints/%s_best.t7' % args.exp_name)

        torch.save(net,'/home/ljm/dcp/checkpoints/%s_%d.t7' % (args.exp_name, epoch))
        gc.collect()

        wandb_log = {}
        wandb_log['RMSE(R)'] = test_r_rmse_ab
        wandb_log['MAE(R)'] = test_r_mae_ab
        wandb_log['RMSE(t)'] = test_t_rmse_ab
        wandb_log['MAE(t)'] = test_t_mae_ab
        wandb.log(wandb_log)
...
def main():
    ...
    test_loader = DataLoader(
        ModelNet40(2048, partition='test', max_angle=max_angle, max_t=max_t, single=-1, partial_source=partial_source,
                   percent=1.0),
    ...
    if args.model == 'dcp':
        ...
        if args.eval:
            if args.model_path is '':
                model_path = '/home/ljm/dcp/checkpoints/' + args.exp_name + '_best.t7'
            else:
